...
namespace text2code
{
    public class text2code
    {
        ...
        public text2code(String text, DataTable dt)
        {       //取得資料庫以及設定待轉換文字
            vocab_phonetic = dt;
            setText(text);
        }
        ...
        public string convert(int times)
        {       //給外部轉換的method 參數times為加密次數
        ...
        private string selectCode(string vocab, string code, int counter)
        {       //在資料庫中搜尋單字以得到正確讀音
            string s = "vocab = '" + vocab + "'";
            DataRow[] dr = vocab_phonetic.Select(s, sortStr);
        ...
        private string selectVocab(string vocab)
        {       //以單一中文字尋找該中文字使用最頻繁的讀音
            string s = "vocab = '" + vocab + "'";
            DataRow[] dr = vocab_phonetic.Select(s, sortStr);
            if (dr.Length < 1) return "error";
            return dr[0][3].ToString();
        }
        private string choosedWordRandomTone(string w)
        {       //以選擇的注音上半部隨機轉換成反切音前段
        ...
        private string choosedToneRandomWord(string w)
        {       //以選擇的注音下半部隨機轉換成反切音後段
            ...
            while (true)
            {
                ...
                int a = rd.Next(0, Shen.Length + 1);
                if (a > 0)
                {
                    t += Shen[(a - 1)];
                }

                int b = rd.Next(0, Mid.Length + 1);
                if (b > 0)
                {
                    t += Mid[b - 1];
                }
        ...
        private string selectRandomWord(string sql)
        {       //以選擇的完整注音轉換成該讀音使用頻率最高的中文字
            if (sql.Length < 1)
            {
                return "error";
            }
            string SQL = "code = '" + sql + "'";
            DataRow[] dr = vocab_phonetic.Select(SQL, sortStr);
            if (dr.Length < 1)
            {
                return "error";
            }
